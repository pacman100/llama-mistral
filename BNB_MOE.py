...
def replace_remaining_with_4bit(model):
    for name, module in model.named_children():
        if (
            ...
            and not isinstance(module, bnb.nn.modules.LinearSparse)
        ...
        if isinstance(module, MoE):
            for expert in module.experts:
                for name, layer in expert.named_children():
                    ...
                    layer = bnb.nn.modules.LinearSparse(
                        in_features,
                        out_features,
                        layer.bias is not None,
                        sparsity_level=0.97,
                    )
                    layer.weight = bnb.nn.modules.ParamsSparse(
                        weight, sparsity_level=0.97
                    )
                    if layer.bias is not None:
                        layer.bias = bias
